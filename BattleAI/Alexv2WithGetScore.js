...
//{p: sum([1 for box in self.game.boxes if self.box_owners.get(box) is p]) for p in self.game.players}
gameState.prototype.getScores = function(){
    Player p = Player(currP);
    return [p.currency, p.territory, p.units];
}
